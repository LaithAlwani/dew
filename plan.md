...
### ⚠ Remaining to fully enable auth
Create a **Clerk JWT template named `convex`** (Clerk dashboard → JWT Templates → Convex) so `ConvexProviderWithClerk` can mint Convex tokens. Optionally enable Google/Apple OAuth in Clerk for the social buttons.
